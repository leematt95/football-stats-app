...
class APITestSuite:
    ...
    def test_endpoint(self, method: str, endpoint: str, expected_status: int = 200,
                     ...
                     test_name: Optional[str] = None) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code != expected_status:
                self.log_test(test_name, "FAIL", response_time, 
    ...
    def test_welcome_endpoint(self) -> None:
        ...
        if success and "message" in data:
            if "Football Stats API" in data["message"]:
                ...
                self.log_test("Welcome Message Content", "FAIL", 0, 
    ...
    def test_player_endpoints(self) -> None:
        ...
        success, data = self.test_endpoint("GET", "/api/players/", 
                                         params={"limit": 5}, 
        ...
        for name in test_names:
            ...
            if success and isinstance(data, list) and len(data) > 0:
                player = data[0]
                print(f"   └─ Found: {player.get('name', 'Unknown')} ({player.get('team', 'Unknown')})")
        
        # Test search with no results
        self.test_endpoint("GET", "/api/players/", 
                          params={"name": "NonExistentPlayer123"}, 
                          test_name="Search non-existent player")
        
        # Test single player by ID
        success, data = self.test_endpoint("GET", "/api/players/1", 
                                         test_name="Get player by ID (1)")
        if success and "name" in data:
            print(f"   └─ Player 1: {data['name']} ({data.get('team', 'Unknown')})")
        
        # Test invalid player ID
        self.test_endpoint("GET", "/api/players/99999", 
                          expected_status=404,
    ...
    def test_search_endpoints(self) -> None:
        ...
        success, data = self.test_endpoint("GET", "/api/players/search/json", 
        ...
        self.test_endpoint("GET", "/api/players/search/json", 
                          expected_status=400,
        ...
        try:
            ...
            if response.status_code == 200 and "text/html" in response.headers.get("content-type", ""):
                ...
                self.log_test("HTML search endpoint", "FAIL", 0, 
    ...
    def test_pagination_endpoints(self) -> None:
        ...
        success, data = self.test_endpoint("GET", "/api/players/paginated", 
        ...
        for per_page in [3, 10, 20]:
            self.test_endpoint("GET", "/api/players/paginated", 
    ...
    def test_error_handling(self) -> None:
        ...
        self.test_endpoint("GET", "/api/nonexistent", 
                          expected_status=404,
                          test_name="Non-existent endpoint")
        
        self.test_endpoint("GET", "/api/players/invalid", 
                          expected_status=404,
                          test_name="Invalid player endpoint")
        
        # Test invalid HTTP methods
        self.test_endpoint("POST", "/api/players/", 
                          expected_status=405,
                          test_name="POST to read-only endpoint")
        
        self.test_endpoint("DELETE", "/api/players/1", 
                          expected_status=405,
    ...
    def test_performance(self) -> None:
        ...
        for endpoint, description in endpoints_to_test:
            ...
            try:
                ...
                self.log_test(f"Performance: {description}", status, response_time, details)
    ...
    def test_data_integrity(self) -> None:
        ...
        success, data = self.test_endpoint("GET", "/api/players/1", 
                                         test_name="Player data structure")
        ...
        if success and isinstance(data, dict):
            ...
            if not missing_fields:
                ...
                print(f"   └─ Player: {data['name']} - {data['position']} - {data['team']}")
            else:
                self.log_test("Player data has required fields", "FAIL", 0, 
        ...
        if success and isinstance(data, list) and len(data) > 0:
            ...
            if isinstance(first_player, dict) and "name" in first_player:
                ...
                self.log_test("Search results structure", "FAIL", 0, 
        ...
        if self.failed_tests > 0:
            ...
            for result in self.test_results:
                if result["status"] == "FAIL":
                    print(f"   • {result['test']}: {result.get('details', 'Unknown error')}")
...
async def test_async_functionality():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get("http://localhost:5000/api/players/?name=Salah") as response:
