...
class TestPlayerEndpoints:
    ...
    @pytest.mark.parametrize("player_name", [
        "Salah", "Haaland", "Kane", "De Bruyne", "Son"
    ])
    def test_search_players_by_name(self, api_session, api_base_url, wait_for_api, player_name):
        """Test searching for players by name."""
        response = api_session.get(f"{api_base_url}/api/players/", params={"name": player_name})
        assert response.status_code == 200
    ...
    def test_search_nonexistent_player(self, api_session, api_base_url, wait_for_api):
        ...
        response = api_session.get(f"{api_base_url}/api/players/", 
...
class TestSearchEndpoints:
    ...
    def test_json_search_by_team(self, api_session, api_base_url, wait_for_api, team_name):
        ...
        for player in data:
            assert team_name.lower() in player["team"].lower() or team_name.lower() in player["name"].lower()
    
    def test_json_search_missing_parameter(self, api_session, api_base_url, wait_for_api):
    ...
    def test_html_search_endpoint(self, api_session, api_base_url, wait_for_api):
        ...
        response = api_session.get(f"{api_base_url}/api/players/search/html", 
                                 params={"name": "Arsenal"})
        assert response.status_code == 200
...
class TestPaginationEndpoints:
    ...
    def test_pagination_basic(self, api_session, api_base_url, wait_for_api, page, per_page):
        """Test basic pagination with different parameters."""
        response = api_session.get(f"{api_base_url}/api/players/paginated", 
    ...
    def test_pagination_with_search(self, api_session, api_base_url, wait_for_api):
        ...
        for player in data["players"]:
            assert "manchester" in player["team"].lower() or "manchester" in player["name"].lower()
...
class TestPerformance:
    ...
    def test_response_times(self, api_session, api_base_url, wait_for_api, endpoint, description):
...
class TestDataIntegrity:
    ...
    def test_search_results_consistency(self, api_session, api_base_url, wait_for_api):
        ...
        for _ in range(3):
            response = api_session.get(f"{api_base_url}/api/players/", params={"name": term})
            assert response.status_code == 200
